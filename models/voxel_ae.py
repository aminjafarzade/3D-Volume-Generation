...
def eval(model, epoch):
    ...
    with torch.no_grad():
        ...
        print(f"Results saved in {path}mlb7TIotMd+2Pg==for epoch {epoch + 1}")
...
def get_batch():
    idxs_table = np.random.randint(low=0, high=table_data_size, size=(1, 22))
    idxs_chair = np.random.randint(low=0, high=chair_data_size, size=(1, 21))
    idxs_airplane = np.random.randint(low=0, high=airplane_data_size, size=(1, 21))
...
path="/root/Diffusion-Project-3DVolume/models/ae_ckpts_updated"
os.makedirs(path, exist_ok=True)
