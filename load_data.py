...
if __name__ == "__main__":
    ...
    zipfile = "hdf5_data.zip"

    shapenet_dir = os.path.join(DATA_DIR, "hdf5_data")

    if not os.path.exists(shapenet_dir):
        print(f"[*] ShapeNet PartSeg dataset will be downloaded under {os.path.join(DATA_DIR, zipfile)}")
        ...
        for split in ls:
            ...
            with mp.Pool(mp.cpu_count()) as pool:
                for r in pool.imap(voxelize, args):
                    ...
                    pbar.update()
            # for arg in args:  # Iterate through the arguments sequentially
            #     r = voxelize(arg)  # Call the voxelize function directly
            #     all_cat_voxels.append(r)  # Append the result to the list
            #     pbar.update()  
